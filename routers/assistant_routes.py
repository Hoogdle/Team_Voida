...
def assistant_category(payload: schemas.AssistantCategory, db: Session = Depends(get_db)):
	...
	task = f"<|사용자 질문|> {payload.voiceInput} <|사용자 질문|>  사용자 질문을 참고해서 주어진 질문이 다음 카테고리 중 무엇에 해당하는지 출력해줘. 카테고리 이름만 사용할거고 다른 내용이 덧붙여지면 오류가 발생하니 카테고리 이름만 출력해줘. <|카테고리|>상품검색,계정설정,카테고리,장바구니,인기상품,할인상품,배송지변경,결제수단 변경, 도움말<|카테고리|>"
